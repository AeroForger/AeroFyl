...
The literal element count must match the declared length in the bootstrap. Indexes have type `int`; indexed reads, indexed writes, and `.length` are supported. The bootstrap performs runtime bounds checks and exits with status 70 on failure.
